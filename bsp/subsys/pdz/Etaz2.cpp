...
uint8_t EtazItem2::num_of_floors()
{
    return 2;
}

uint8_t EtazItem2::getErrorPacket()
{
    return cBfppClass.get_error_packet();
}

uint8_t EtazItem2::getSendAddress()
{
...
uint8_t EtazItem2::EtazDrv()
{
    ...
    if (BFPP_INFO->msk1 & BFPP_FIRE_FLOOR2)
        floor2.set_fire(FromPK);
    ...
    if (floor1.have_fire()) {
        drv_YA1();
    } else if (floor2.have_fire()) {
        drv_YA2();
    }

// increment(); // увеличиваем CurrentFloor на одну позицию

    return out;
}

void EtazItem2::show_mnem()
{
    ...
    if (floor1.have_fire() || floor2.have_fire()) {
        temp = B_RED_LAMP;
    }
...
uint8_t EtazItem2::get_current_floor()
{
    return CurrentFloor;
}
...
*/
uint8_t EtazItem2::return_CurrentFloor()
{
    return CurrentFloor;
}
BfppFloorCounter EtazItem2::return_CurrentBfppFloor()
{
    return CurrentBfppFloor;
}

void EtazItem2::set_next_fire_floor(BfppFloorCounter & bfpp_counter)
{
    ...
    //
    // ну собстно, походу я именно так и поступил

    if (bfpp_counter.floor_counter >= num_of_floors())
        bfpp_counter.floor_counter = num_of_floors() - 1;

    if (KEY_5) {
        switch (bfpp_counter.floor_counter) {
            case 0:
                bfpp_counter.floor_counter = 1;
                break;
            case 1:
...
void EtazItem2::show_menu(uint8_t cur_floor)
{
    LCD.init();
...
uint8_t EtazItem2::set_fire(eFireSource state)
{
    switch (BfppCount.floor_counter) {
        case 0:
...
void EtazItem2::BUPK_WorkDrv()
{
    floor1.set_event(PkNormal);
    floor2.set_event(PkNormal);
