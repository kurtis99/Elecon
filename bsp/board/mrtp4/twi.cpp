...
unsigned char TWI::i2c_start(void)
{
	//TWI transmit start
	TWCR = (1<<TWINT)|(1<<TWSTA)|(1<<TWEN);//send start condition

	while(!(TWCR & (1<<TWINT)))
        {};		//wait for TWINT flag set to indicate start condition has been transmitted
...
unsigned char TWI::i2c_repeatStart(void)
{
	TWCR = (1<<TWINT)|(1<<TWSTA)|(1<<TWEN); 	//Send START condition
    while (!(TWCR & (1<<TWINT)))
        {};   			//Wait for TWINT flag set. This indicates that the
		  										//START condition has been transmitted
    if ((TWSR & STATUS_MASK) == REPEAT_START)	//Check value of TWI Status Register
 		return(0);
	else
		return(1);
}
unsigned char TWI::i2c_sendAddr(unsigned char addr)
{
	...
	while(!(TWCR & (1<<TWINT)))
        {};	//wait for TWINT flag set to indicate that SLA_W has been transmitted
...
unsigned char TWI::i2c_sendData(unsigned char data)
{
	...
	while(!(TWCR & (1<<TWINT)))
        {};		//wait for TWINT flag set to indicate data has been transmitted
...
unsigned char TWI::i2c_receiveData_ACK(void)
{
  unsigned char data;
...
unsigned char TWI::i2c_receiveData_NACK(void)
{
  unsigned char data;
